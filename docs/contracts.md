...
| `ProvenanceRecord` / `provenance_records` | `id`, `lot_id`, `payload_hash`, `chain_id`, `tx_hash`, optional `ens_name` | Inherits lot `org_id`; submitter actor is recorded | `pending → submitted → confirmed` or `failed`; audit subject `ProvenanceRecord:id`. | The hashed payload and prior attempts are immutable. Retry appends a new attempt or an audited status event; no confirmed hash is overwritten. |
